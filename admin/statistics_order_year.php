    ...
    <div class="container">
        ...
        <div class="main-content main-content-order">
            ...
            <div class="sale-statistics-nav">
                ...
                <a style="background-color: #01640d" href="statistics_year.php">Theo năm</a>
            </div>
            <form action="statistics_order_year.php" method="POST" style="display: inline-block;">
                <input name="statistics-y" style="margin: 20px 0 0 0;" type="year" placeholder="Tìm kiếm theo năm...">
                <button name="statistics-y-btn" style="min-width: 40px; padding: 12px 12px; border-radius: 20px; background-color: green;"><i class="fa fa-search"></i></button>
            </form>
            ...
            <?php
                $date = $_POST["statistics-y"].'-01-01';
                if ($_POST["statistics-y"] == "") {
                    header("location: statistics_year.php");
                }
            ?>
            <?php
                include('dbconnection.php');
                $query = 'SELECT * FROM (
                    SELECT Z.year, sum(Z.sale) as y_sale, sum(Z.order_num) as y_ordernum FROM
                    (SELECT LASTTABLE.order_date, LASTTABLE.day, LASTTABLE.month, LASTTABLE.year, sum(LASTTABLE.total) AS sale, count(*) AS order_num
                                    ...
                                    SELECT orderID, total, order_date, order_status, DAY(order_date) AS day, MONTH(order_date) AS month, YEAR(order_date) AS year
                                        ...
                                        ) WHERE order_status = "Hoàn thành" ) LASTTABLE GROUP BY LASTTABLE.order_date) Z GROUP BY Z.year) A WHERE A.year='.$_POST["statistics-y"];
            ...
            ?>
                <tr>
                    <td style="font-weight: bold"><?php echo "Năm ".$row["year"]?></td>
                    <td style="background-color: #f8ffd4"><?php echo number_format($row["y_ordernum"])?></td>
                    <td style="background-color: #f5ffc1"><?php echo number_format($row["y_sale"])."vnđ"?></td>
                </tr>
            <?php
            }
            mysqli_close($link);
