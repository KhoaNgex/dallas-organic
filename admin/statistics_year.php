    ...
    <div class="container">
        <?php
            ...
            if(isset($_POST["statistics-sort-btn"])) {
                if($_POST["statistic-sort"] == 1) {
                    $order = 'ORDER BY y_sale ASC';
                }
                if($_POST["statistic-sort"] == 0) {
                    $order = 'ORDER BY y_sale DESC';
                }
            }
        ?>
        <?php 
        ...
        <div class="main-content main-content-order">
            ...
            <?php
                ...
                $query = 'SELECT Z.year, sum(Z.sale) as y_sale, sum(Z.order_num) as y_ordernum FROM
                (SELECT LASTTABLE.order_date, LASTTABLE.day, LASTTABLE.month, LASTTABLE.year, sum(LASTTABLE.total) AS sale, count(*) AS order_num
                                ...
                                SELECT orderID, total, order_date, order_status, DAY(order_date) AS day, MONTH(order_date) AS month, YEAR(order_date) AS year
                                    ...
                                    ) WHERE order_status = "Hoàn thành" ) LASTTABLE GROUP BY LASTTABLE.order_date) Z GROUP BY Z.year '.$order;
